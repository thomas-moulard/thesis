...
\cite{thrun2005probabilistic}. Les travaux de navigation en robotique
...
Kuffner, Joel Chestnutt, Koichi Nishiwaki -- \begin{CJK*}{UTF8}{min}正
  西脇 光一\end{CJK*} --
  \cite{05humanoids.michel,Ozawa05smc,Chestnutt03planningbiped,nishiwaki2002online}
  de l'Université de Freiburg en Allemange
  \cite{osswald2010learning}. Dans le domaine la vision pour les
  humanoïdes, l'utilisation de l'asservissement visuel a été tenté
  \cite{Dune10iros} ainsi que des techniques de SLAM telles que
  \cite{Stasse06iros,kwak20093d}. Une introduction à l'asservissement
  visuel est \cite{chaumette2006visual,chaumette2007visual}. La
  possibilité d'utiliser un robot humanoïde pour la modélisation
  automatique d'objets 3D a également été envisagée par
  \cite{foissotte2009two,stasse2008treasure}. Une approche fondée sur
  la replanification à partir de données vision sur HRP-2 a également
  été publié dans \cite{dang2011framework}.


\section{Description d'un mouvement robotique complexe}


Décrire le comportement d'une pile de tâches revient à décrire deux
...
de $\text{SE}(3)$. Les éléments de $\text{SE}(3)$ peuvent s'exprimer
...
Le scénario proposé est le suivant: HRP-2 contourne un obstacle, passe
