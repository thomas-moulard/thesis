...
le raisonnement est ensuite fondé sur la différentiabilité de cette
fonction de coût. Plus simplement, près d'une mauvaise solution, il
n'y aura que des solutions légèrement meilleures et légèrement pires,
mais jamais trop différentes. De ce fait, il ``suffit'' de chercher
dans quelle direction aller pour se diriger vers les meilleures
solutions pour finir par les trouver. Cette approche implique une
limitation majeure: si la fonction de coût n'est pas convexe, on ne
trouvera pas forcément la meilleure solution globale -- parmi toutes
les solutions -- mais la meilleure solution locale. C'est-à-dire qu'au
voisinage de cette solution, toutes les autres solutions sont de moins
bonne qualité. Dans cette situation, tous les solveurs arrêtent leur
...
spécifiant que telle ou telle valeur encodant la position d'un degré
de liberté doit rester dans un intervalle donné.


En exprimant comment résoudre un problème pratique via la construction
d'une fonction de coût $f$ et de contraintes $g_i > 0$, on
...
\begin{equation}
  \min_{\mathbf{x} \in \mathbb{R}^n} f(\mathbf{x}) \text{ sous la contrainte } \mathbf{x} \in \mathbf{X}
\end{equation}
...
\hline
\textit{Classe de problèmes} & \textit{Fonction de coût} & \textit{Contraintes}\\
\hline
...
pour ce faire? Le Tableau \ref{tab:chap1_soft} détaille une partie
...
\hline
\textbf{\href{http://devernay.free.fr/hacks/cminpack/index.html}{C/C++ MinPack}} &
Moindres carrés non linéaires & C/C++, réentrant, thread-safe \\
\hline
...
\end{tabular}
\end{center}
\caption{Zoologie des logiciels en optimisation numérique. \label{tab:chap1_soft}}
\end{table}
%
...
différentiables et continues deux fois? Une fois? Est-il certain de ne
...
devenu un enjeu critique. Pour ce faire, les langages de programmation
ont progressivement adopté le typage comme moyen de n'autoriser que la
construction de programmes valides. Cette notion de type est issue de la
...
\index{$\lambda$-calcul} inventé par Alonzo Church \index{Alonzo
  Church} dans les années trente. Ce système de calcul formel a une
expressivité équivalente à une machine de Türing\index{machine de
  Türing}. Nous allons tout d'abord nous concentrer sur la
construction des lambda-termes \index{lambda-terme}, c'est-à-dire
l'ensemble des expressions syntaxiquement correctes pouvant être
construites en lambda-calcul. Ces lambda-termes peuvent être divisés
en trois catégories: variables, applications et abstractions. Les
variables: $x$, $y$, \ldots Les applications sont l'ensemble des
expressions $u v$ où $u$ et $v$ sont deux lambda-termes. Enfin les
abstractions sont les expressions du type: $\lambda x.u$ où $x$ est
une variable et $u$ un lambda-terme. Par exemple, l'application
identité peut s'écrire en lambda-calcul: $\lambda x.x$.
...
dans Simula I\index{Simula I} au cours des années soixante. Ce langage
avait pour but de permettre le développement de simulation à
événements discrets. Ce paradigme de pensée s'est progressivement
développé jusqu'à être massivement adopté par la majorité des langages
de programmation au cours des années quatre-vingt-dix. L'adoption
massive du C++ (Bjarne Stroustrup \index{Bjarne Stroustrup}, 1983) a
participé à la démocratisation de la POO.
...
\begin{mydef}\label{chap1_oo_app}
  ...
  Dès lors que $\tau_3$ est un sous-type de $\tau_1$, les fonctions
...
les catégories selon leur nature: continue, différentiable, etc.
...
\begin{mydef}\label{def:chap1_fcn}
  ...
  la fonction et la méthode $\text{calcule}$ permet d'évaluer la
...
\begin{mydef}\label{def:chap1_nderivfcn}
  ...
  classe sous la forme d'une chaîne où $\mathcal{C}^n <:
...
définir sa fonction $F$ de la faire hériter du type adéquat de la
...
\begin{mydef}\label{def:chap1_prob}
  ...
  $\tau_i <: \text{Fonction}$, $i \in \{1, \dotsc, n\}$:
  ...
  chaque instance fera partie de cet ensemble de types.
\end{mydef}
...
Afin de démontrer que le formalisme décrit dans la section
...
\begin{mydef}\label{ex:chap1_quadfct}
  Soit $\text{FonctionQuadratique}$ un type tel que:
  $$\text{FonctionQuadratique} <: \text{Fonction}_2$$
\end{mydef}

\begin{mydef}\label{ex:chap1_linearfct}
  Soit $\text{FonctionLinéaire}$ un type tel que:
  $$\text{FonctionLinéaire} <: \text{FonctionQuadratique}$$
\end{mydef}


Nous avons ici choisi de faire dériver les fonctions linéaires et
quatratiques\index{fonction quadratiques} du type
$\text{FonctionQuadrique}_2$. Cela ne correspond pas à la réalité
mathématique dans la mesure où une fonction linéaire est infiniment
dérivable, la dérivée étant nulle à partir de la seconde
dérivation. Il n'est pas possible d'exprimer cet état de fait au
regard de la modélisation adoptée ce qui représente une limite du
modèle de typage proposé. En pratique, les fonctions plus de deux fois
dérivables ne présentent pas d'intérêt particulier et cette limite ne
pose donc pas de problèmes en pratique.
...
éventuellement choisir à dessein un solveur ``trop'' puissant pour le
...
pour s'apercevoir qu'il serait bien mieux traité à l'aide d'un
...
la \autoref{sec:chap1_roboptim}. Concernant les limites de la
...
alors plus un tableau de réels, mais un n-uplet contenant des éléments
...
visible lorsque l'on travaille sur des rotations en trois dimensions,
$SO(3)$, pour lesquels de nombreuses représentations existent. Par
...
pas les problèmes! Pour changer de solveur, il faut donc instancier un
type de problème différent et ensuite le passer au nouveau
solveur. Cela représente généralement un travail court et facile, mais
qui peut désarçonner les utilisateurs peu adeptes des langages de
programmation utilisant des règles de typage complexe.
...
\item[CFSQP\index{CFSQP}] solveur pour les problèmes non linéaires,
  contraintes égalités et inégalités, calcul du gradient requis
  \citep{97craig},
...
\section[Scénario d'utilisation]{Scénario d'utilisation: écriture d'une application robotique}
...
SO(2)$ des translations et rotation en deux dimensions tout en
...
passe réalisée par un algorithme d'optimisation. L'utilisation d'une
méthode probabiliste permet d'éviter de ``tomber'' dans des minima
locaux ce qui est un problème récurrent des méthodes d'optimisation.
C'est la seconde passe, permettant de raffiner la trajectoire, que
nous avons choisi d'exprimer dans le paradigme de RobOptim.
...
appelé B-spline\index{spline!B-spline} est défini comme une
...
\begin{mydef}\label{def:chap1_coxdeboor}
  ...
  \geq 1$, le degré des polynômes considérés. Soit $\Xi = (t_0,
  \dotsc, t_{n+k-1})$ un vecteur appelé, vecteur nodal avec $t_0 \geq
  \dotsc \geq t_{n+k-1}$. Les valeurs $t_i$ sont appelées noeuds. Les
  fonctions de base $N_{i,k}$ que nous allons définir, et donc les
  courbes B-splines que nous construirons ensuite, sont polynomiales
  sur chaque intervalle $[t_i,t_{i+1}]$, pour $i = 0,\dotsc,n-2$.
...
\begin{mydef}\label{def:chap1_spline}
  ...
  courbe B-spline Q d'ordre $n$ et de degré $k - 1$ ayant $\Xi$ pour
...
notre cas, cette valeur sera notée $\lambda$ et fera partie des
...
d'optimisation ainsi constitué est illustré par la
\autoref{tab:param}.
...
de vitesse détaillée ci-dessous, soit saturée. La \autoref{fig:speed}
...
en privilégiant la marche en avant ou en arrière par rapport à la
marche sur le côté. Outre l'aspect anthropomorphe, les robots
humanoïdes ont un comportement dégradé quand ils marchent ``en
crabe''. L'espace accessible par les jambes est réduit dans cette
direction et la forme des semelles du robot a tendance à le faire
glisser dans ce cas. Les contraintes sur les vitesses sont dupliquées
afin de s'appliquer à la fois au pied gauche et au pied droit. La
formulation de la contrainte pour tous les points de la trajectoire
...
\begin{table}
  \begin{center}
    \begin{tabular}{|>{\small}p{.25\textwidth}|l l l|l|}
    ...
    \end{tabular}
  \end{center}
  \caption{Temps de calculs. Le temps de calcul complet comprend la
...
objet pour la résolution de problèmes d'optimisation numérique ainsi
