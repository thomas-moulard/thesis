...
exemple, on peut augmenter le niveau de différentiabilité d'une
...
l'utilisation du ``Design Pattern'' décorateur tel que décrit dans le
livre de référence~\citep{design.pattern}. Ce motif de conception a la
...
résolution, soit utiliser le système de plug-in permettant d'en
charger à l'execution. L'objectif de ce système est de pouvoir
complètement déléguer au système la résolution d'un problème: il y a
un certain nombre de solveurs disponibles sur le système et l'on
pourrait ainsi, avoir une sélection automatique de l'algorithme
...
splines, c'est à dire de courbes paramétrées par des points de
contrôle. Il est alors naturel de tenter de vouloir altérer ces points
de contrôle afin de rafiner le comportement de la trajectoire: tenter
de minimiser le temps nécessaire pour réaliser le mouvement, ou
l'énergie ou tout autre critère adéquat.

Cette bibliotèque fournit des bornes sur les vitesses du système à la
...
d'un point A à un point B. Ce dernier problème se ramène lui-même à
trouver le mouvement sans collision dans le plan d'une boîte en
...
--. Cet arbre va tenter d'échantillonner l'espace $\mathrm{R}^2 \times
S^1$ des translations et rotation en deux dimensions tout en validant
...
définies sous la forme d'une spline cubique. Une spline est une courbe
paramétrée qui est définie comme une combinaison linéaire d'une
famille de fonctions de base. Les deux définitions ci-dessous
permettent de calculer analytiquement ces courbes et sont tirées
de~\citep{algo.3d}

\begin{mydef}\label{def:chap1_coxdeboor}
  Soit $n \in \mathrm{N}$, le nombre de points de contrôles. Soit $k
  \geq 1$, le degré des polynômes considérés. Soit $T = (t_0, \dotsc,
  ...
  $N_{j,r-1}$ s'appelle la formule de Cox-de-Boor. Lorsque les
  dénominateurs de la formule s'annulent, on pose $\frac{0}{0} =
  0$. C'est le cas lorsque plusieurs n\oe uds $t_i$ sont confondus.
...
Les variables d'optimisation consiste en le vecteur de paramètres d'une
trajectoire temps minimal tel qu'illustré par la \autoref{fig:param}.
...
\end{tabular}
  \end{center}
  \caption{Variables d'optimisation du problème. \label{fig:param}}
\end{figure}

\subsubsection{Fonction de coût}

L'approche choisie consiste a garder une fonction de coût simplifiée
au maximum: l'évaluation de la fonction de coût retourne simplement le
facteur d'accélération de la trajectoire temps minimal.

\begin{eqnarray}
Cost (x) & = & (1, 0, \ldots, 0) x
\end{eqnarray}


En pratique, cela signifie que le solveur va tenter de trouver un
vecteur de paramètre permettant une vitesse d'exécution du mouvement
maximum. Il s'agira donc de saturer la contrainte de vitesse tout en
continuant a satisfaire la contrainte sur les obstacles. La
\autoref{fig:speed} montre le processus de saturation d'une contrainte
sur la vitesse dans le cadre d'une spline unidimensionnelle.
...
Le but de la contrainte sur les vitesses est double: garder une
trajectoire utilisable et privilégier la marche en avant par rapport a
la marche sur le cote. Outre l'aspect anthropomorphique, quand le
robot marche en pas chasses, il glisse ce qui résulte en un décalage
entre l'environnement réel et virtuel.


Les contraintes sur les vitesses s'appliquent soit au pied gauche,
soit au pied droit du robot (\autoref{fig:boxoptim}).


La formulation de la contrainte est:

\begin{eqnarray}
SpeedConstraint(x) & = & \frac{1}{2} (\alpha v_f^2 + \beta v_{orth}^2)
\end{eqnarray}

$v_f$ et $v_{orth}$ étant respectivement les vitesses frontales et
orthogonales du pied considéré.  $\alpha$ et $\beta$ déterminent à
quel point la marche vers l'avant ou la marche latérale doit être
préférée. Durant la phase expérimentale, les valeurs utilisées ont été
$\alpha = 1$ et $\beta = 2$.


Le calcul de la contrainte se fait en deux étapes:
\begin{itemize}
\item calcul de la vitesse du pied gauche et droit à partir de la
vitesse du robot,
\item calcul des vitesses frontales et orthogonales séparément.
\end{itemize}


Le calcul de la vitesse des pieds est réalisée en utilisant le torseur
exprimant le champs des vitesses d'un solide:

\begin{eqnarray*}
\vec{v_{left}} & = & \vec{v} + \vec{\Omega} \wedge \vec{CL}\\
\vec{v} & = & \left(
\begin{array}{c}
  \dot{x}\\
  \dot{y}\\
...
\vec{CL} & = & \left(
\begin{array}{c}
  -sin~\theta ~ \frac{\Delta feet}{2}\\
  -cos~\theta ~ \frac{\Delta feet}{2}
\end{array}
\right)\\
%
\vec{v_{left}} & = & \left(
\begin{array}{c}
  \dot{x} - \dot{\theta} ~ cos~\theta ~ \frac{\Delta feet}{2}\\
  \dot{y} - \dot{\theta} ~ sin~\theta ~ \frac{\Delta feet}{2}
m\end{array}
\right)
\end{eqnarray*}

$\vec{CL}$ est le vecteur entre $C$ et $L$, respectivement le centre
de la boite englobante du robot et le cendre de son pied
gauche. $\Delta feet$ est la distance séparant ses deux pieds, il
s'agit d'un paramètre connu du problème.


En remplaçant $\vec{CL}$ par $\vec{CR}$ ou $R$ est le centre du pied
droit, on peut calculer la vitesse du pied droit:
...
On peut en déduire la formule générale en ajoutant un paramètre $\epsilon$ valant $-1$
pour le pied gauche et $1$ pour le pied droit:

\begin{eqnarray}
\vec{v_{foot}} & = & \left(
\begin{array}{c}
  \dot{x} + \epsilon ~ \dot{\theta} cos~\theta ~ \frac{\Delta feet}{2}\\
  \dot{y} + \epsilon ~ \dot{\theta} sin~\theta ~ \frac{\Delta feet}{2}
\end{array}
\right)
\end{eqnarray}


Une fois la vitesse des pieds calcules, on déduit a partir des formules de vitesse de la section précédente:
...
Cette contrainte force donc le robot à réaliser un mouvement naturel
(c'est à dire en privilégiant la marche en avant) et suffisamment lent
pour être jouable sur le robot. Afin d'être certain que la contrainte
est vérifiée en permanence, le temps est discrétisé et on ajoute la
contrainte pour tous les pas de temps.

\begin{figure}[htbp]
  \begin{center}
\includegraphics[width=5cm]{src/chap1-roboptim/box-optimization.pdf}
  \end{center} \caption{Vitesses séparéees du pied gauche et du pied
...
souhaite simplement maintenir une distance minimale entre le robot et
chaque obstacle a tout moment. Comme avec la contrainte précédente, on
discrétise le temps afin de vérifier régulièrement la contrainte.

La contrainte est la suivante:

\begin{eqnarray}
\forall j,  Distance(R, O_j) \geq d_{min}
\end{eqnarray}

$d_{min}$ est moitié de la dimension la plus grande de la boite englobante.


\subsection{Construction du problème et résolution}
...
Ce problème d'optimisation a donné des résultats probants comme le
montre la \autoref{fig:obstacle}. Cependant, un pré-traitement
utilisant une méthode d'optimisation géométrique a été nécessaire afin
d'éviter de faire démarrer le solveur d'une trajectoire trop
sous-optimale. Cela engendre un temps de calcul très long et parfois
des trajectoires inattendues a cause de minima locaux. Cependant, dans
tous les cas, la résolution du problème reste encore trop long pour
pouvoir en généraliser l'usage: diminuer le temps de résolution est
toujours un problème important. Si de nombreuses optimisations
informatique sont encore possible, elles restent encore à implémenter.


\begin{figure}[htbp]
  \begin{center}
\includegraphics[width=\linewidth]{src/chap1-roboptim/straight-line-obstacle.png}
  \end{center} \caption{Exemple d'optimisation d'une trajectoire en
  présence d'obstacles. \label{fig:obstacle}}
\end{figure}


Une trajectoire résultant d'un problème d'optimisation a également été
exécuté sur le robot avec succès \autoref{fig:demo}. Il s'agissait
d'aller du point $(0, 0, \pi/2)$ vers le point $(0, 3, -\pi/2)$. La
trajectoire optimisée comporte une rotation de 180 degrés pendant que
le robot continue à avancer.
