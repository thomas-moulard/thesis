...
\begin{mydef}
  Un arbre cinématique\index{arbre cinématique} est un ensemble de
  joints\index{joint} formant un arbre.  Soit $\mathcal{A}$ un arbre
  cinématique, on a:
...
Ce modèle dît du ``Pendule Inverse Linéarisé''\index{Pendule Inverse
...
\cite{06lavalle}. Les fondements mathématiques de la robotique sont
très clairement décrits par \cite{94murray}.
...
contraintes\index{Contrainte inégalité (optimisation)} inégalités
