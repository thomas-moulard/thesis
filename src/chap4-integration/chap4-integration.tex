...
composants ne font pas partis du même processus ou ne sont pas
...
à une vitesse supérieure doivent être intégrées à la boucle de
...
nuages de points 3d tel que PCL -- la Point Cloud Library --\index{PCL
  (Point Cloud Library)}.
...
posteriori. Des systèmes de surveillance des valeures critiques sur
les couples, les impacts sur les capteurs de force ont donc été mis en
...
maximum qui sont soit le résultat de la conception mécanique du
joint, soit lié à la géométrie du robot: une valeur différente
entraînerait immédiatement une autocollision et il n'est donc pas
...
liberté combinés et qui ne peut donc pas être évitée en spécifiant
les bornes des degrés de liberté est une autocollision qui doit être
prise en compte.
...
être enregistrée afin d'assurer que le contact ne va pas compromette
...
valeurs numériques elles-même sont toujours difficiles à détecter. On
...
on sera donc contraint par l'interface des composants pré-existant que
...
composants acccédants aux informations équivalentes simulées et le
